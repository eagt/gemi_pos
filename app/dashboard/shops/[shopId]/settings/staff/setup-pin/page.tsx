...
import { setStaffPin, getUserOtherBusinessPins, copyPinFromOtherBusiness } from './actions'
import { ShieldCheck, Loader2, Building2, Copy } from 'lucide-react'
...
export default function SetupPinPage({ params }: { params: Promise<{ shopId: string }> }) {
    ...
    console.log('[SetupPinPage] Rendered. Step:', step)

    useEffect(() => {
        console.log('[SetupPinPage] Mounted')
        loadOtherBusinesses()
    }, [])

    const loadOtherBusinesses = async () => {
        try {
            console.log('[SetupPinPage] Loading businesses...')
            const { shopId } = await params
            const businesses = await getUserOtherBusinessPins(shopId)
            console.log('[SetupPinPage] Businesses loaded:', businesses.length)
            setOtherBusinesses(businesses as any)

            // If no other businesses, skip choice step
            if (businesses.length === 0) {
                console.log('[SetupPinPage] No other businesses, going to enter step')
                setStep('enter')
            }
        } catch (error) {
    ...
    const handleReusePinFrom = async (sourceStaffId: string, businessName: string) => {
        console.log('[SetupPinPage] Reusing PIN from:', sourceStaffId)
        setLoading(true)
        try {
            const { shopId } = await params
    ...
    const handlePinComplete = (value: string) => {
        console.log('[SetupPinPage] Pin complete. Value length:', value.length, 'Step:', step)
        setError(false)
        if (step === 'enter') {
            setPin(value)
            setTimeout(() => {
    ...
    const handleSubmit = async () => {
        console.log('[SetupPinPage] Submit clicked. Pin:', pin, 'Confirm:', confirmPin)
        if (pin !== confirmPin) {
        ...
        try {
            const { shopId } = await params
            console.log('[SetupPinPage] Calling setStaffPin for shop:', shopId)
            const result = await setStaffPin(shopId, pin)
            console.log('[SetupPinPage] setStaffPin result:', result)
            ...
            console.error('[SetupPinPage] Error calling setStaffPin:', error)
